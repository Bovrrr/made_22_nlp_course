...
class TfIdf(TransformerMixin):
    ...
    def fit(self, X: np.ndarray, y=None):
        ...
        :param X: array of texts to be trained on
        """
        # # of documents in train
        num_of_docs = len(X)

        # set of words in train
        words = set()
        for text in X:
            words = words.union(text.split())

        # word and number of this word in whole train
        word_count = {word: 0 for word in words}
        for text in X:
            for word in text.split():
                word_count[word] += 1
        # list of top k popular words
        self.top_k_words = (
            [
                word
                for word, count in sorted(
        ...
        for word in words:
            for text in X:
                if word in text.split():
                    df[word] += 1
        for word in words:
            self.idf[word] = np.log(num_of_docs / df[word])
